...
class Player(Block):
    ...
    def update(self):
        ...
        self.rect.x = pos[0]
        self.rect.y = pos[1]
        pygame. mouse.set_visible(False) #чтобы не было видно мышки 
 
# Initialize Pygame
pygame.init()

pygame.display.set_caption("Hungry Lion Game")
...
for i in range(35):
    ...
    block_2 = Block(RED, 15, 15) # размеры 
    ...
    block.change_x = random.randrange(-1, 1) #-3 4 -> СКОРОСТЬ 
    block.change_y = random.randrange(-1, 1)
    ...
    block_list.add(block)  # зеленый 
    block_list.add(block_2) # красный 
